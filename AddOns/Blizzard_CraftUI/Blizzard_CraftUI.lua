		...
		if ( craftIndex <= numCrafts ) then	
			...
			if ( craftType == "header" ) then
				...
				if ( craftSubSpellName ~= "" ) then
					craftButtonSubText:SetFormattedText(PARENS_TEMPLATE, craftSubSpellName);
				else 
					craftButtonSubText:SetText("");
				end
				if ( trainingPointCost > 0 ) then
					craftButtonCost:SetFormattedText(TRAINER_LIST_TP, trainingPointCost);
				else
...
function Craft_UpdateTrainingPoints()
	local totalPoints, spent = GetPetTrainingPoints();
	if ( CraftIsPetTraining() and totalPoints > 0 ) then
...
function CraftFrame_PlaytimeUpdate()
	if ( PartialPlayTime() ) then
		CraftCreateButton:Disable();
		if (not CraftCreateButtonMask:IsShown()) then
			CraftCreateButtonMask:Show();
			CraftCreateButtonMask.tooltip = format(PLAYTIME_TIRED_ABILITY, REQUIRED_REST_HOURS - floor(GetBillingTimeRested()/60));
		end
	elseif ( NoPlayTime() ) then
		CraftCreateButton:Disable();
		if (not CraftCreateButtonMask:IsShown()) then
			CraftCreateButtonMask:Show();
			CraftCreateButtonMask.tooltip = format(PLAYTIME_UNHEALTHY_ABILITY, REQUIRED_REST_HOURS - floor(GetBillingTimeRested()/60));
		end
	else
		if (CraftCreateButtonMask:IsShown()) then
			CraftCreateButtonMask:Hide();
			CraftCreateButtonMask.tooltip = nil;

			CraftFrame_SetSelection(CraftFrame.selectedSkill);
			CraftFrame_Update()
		end
	end
end