...
function GuildNews_Update(frontPage, numButtons)
	...
	for i = 1, numButtons do
		...
		if ( index == 0 ) then
			button.text:SetPoint("LEFT", button.dash, "RIGHT", 0, 0);
			button.icon:Show();
			...
			button.index = nil;
			button.newsType = nil;
			-- if truncated, set a fake newsType to get a tooltip
			if ( button.text:GetStringWidth() > button.text:GetRight() - button.text:GetLeft() ) then
		...
		elseif ( index <= numGuildNews ) then
			...
			if ( isHeader ) then
				...
				button.text:SetPoint("LEFT", button.dash, "LEFT", 0, 0);
...
function GuildNewsButton_OnEnter(self)
	...
	elseif ( newsType == NEWS_PLAYER_ACHIEVEMENT ) then
		...
		GameTooltip:Show();
	elseif ( newsType == NEWS_DUNGEON_ENCOUNTER ) then
		local isSticky, isHeader, newsType, text1, text2 = GetGuildNewsInfo(self.index);
		GuildNewsButton_AnchorTooltip(self);
		GameTooltip:SetText(text2);
		local zone = GetRealZoneText(self.data);
		if ( zone ) then
...
function GuildNewsDropDown_Initialize(self)
	...
	-- we don't have any options for this combination
	if ( newsType == NEWS_DUNGEON_ENCOUNTER and not CanEditMOTD() ) then
		return;
	end
	...
	if ( newsType == NEWS_PLAYER_ACHIEVEMENT ) then
