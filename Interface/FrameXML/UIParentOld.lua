	...
	for _, flag in pairs(yOffsetFrames) do
		...
		if ( flagValue ) then
			if ( flagValue == "variable" ) then
				...
				if ( flag == "bottomEither" or flag == "bottomLeft" or flag == "bottomRight" ) then
					-- pass false to not include main menu bar when getting height since this stuff anchors to top of the main menu bar
					flagValue = EditModeManagerFrame:GetBottomAnchoredActionBarHeight(false)
				end
...
function FramePositionDelegate:UIParentManageFramePositions()
	UIPARENT_VARIABLE_OFFSETS["rightActionBarsX"] = EditModeManagerFrame:GetRightAnchoredActionBarWidth();
	...
	if ( PlayerPowerBar and not PlayerPowerBar:IsUserPlaced() ) then
...
UIParentBottomManagedFrameMixin = { }; 
function UIParentBottomManagedFrameMixin:OnShow()
	UIParent_ManageFramePositions();
end 

function UIParentBottomManagedFrameMixin:OnHide()
	UIParent_ManageFramePositions();
end 