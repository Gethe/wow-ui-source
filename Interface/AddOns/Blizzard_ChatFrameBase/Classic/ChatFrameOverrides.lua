...
function ChatFrameMixin:SystemEventHandler(event, ...)
	...
	elseif ( event == "PLAYER_LEVEL_UP" ) then
		ChatFrameUtil.DisplayLevelUp(self, ...)
		return true;
...
function ChatFrameMixin:OnUpdate(elapsedSec)
	...
	if ( flashTimer < ChatFrameConstants.ScrollToBottomFlashInterval ) then
		self.flashTimer = flashTimer;
		return;
	end

	while ( flashTimer >= ChatFrameConstants.ScrollToBottomFlashInterval ) do
		flashTimer = flashTimer - ChatFrameConstants.ScrollToBottomFlashInterval;
	end
	self.flashTimer = flashTimer;
