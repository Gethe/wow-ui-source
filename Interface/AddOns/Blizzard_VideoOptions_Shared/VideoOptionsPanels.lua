	...
	if(isValid) then
		...
		if(self.type == CONTROLTYPE_DROPDOWN) then
			...
			for fallbackIndex = dropdownIndex - 1, 1, -1 do
				local fallbackValid = IsValid(self, fallbackIndex);
...
function Graphics_TableGetValue(self)
	...
	for key, value in ipairs(self.data) do
		...
		if(match==true and value.notify ~= nil) then
			for notify_key, notify_value in pairs(value.notify) do
				if(_G[notify_key] == nil) then
					return nil;
				end
				if(_G[notify_key].GetNewValueString) then
					local v = _G[notify_key]:GetNewValueString();
					if(v == nil) then
