...
function PTR_IssueReporter.AttachDefaultCollectionToSurvey(survey, ignoreTypeQuestion, setAsFeedback)
	...
	survey:AddDataCollection(collector.RunFunction, GetAreaID)
...
function PTR_IssueReporter.CreateReports()
	...
	local GetDecorIDFromGUID = function(dataPackage)
		if (dataPackage) and (dataPackage.ID) then
			local lines = {}
			local guid = dataPackage.ID
			for s in guid:gmatch("[^-]+") do
				table.insert(lines, s)
			end
			if (lines[4]) then
	...
	local getComponentType = function(dataPackage)
		local componentType = 0
		if dataPackage and dataPackage.Additional and dataPackage.Additional.type then
			componentType = dataPackage.Additional.type
		end
		
		return componentType
	end

	local getComponentThemeSet = function(dataPackage)
		local themeSet = 0
		if dataPackage and dataPackage.Additional and dataPackage.Additional.currentThemeSet then
			themeSet = dataPackage.Additional.currentThemeSet
		end
	...
	local getComponentDoorType = function(dataPackage)
		local doorType = 0
		if dataPackage and dataPackage.Additional and dataPackage.Additional.doorType then
			doorType = dataPackage.Additional.doorType
		end

		return doorType
	end
	
	local decorReport = PTR_IssueReporter.CreateSurvey(22, "Issue Report: %s")
	PTR_IssueReporter.AttachDefaultCollectionToSurvey(decorReport, true)
	decorReport:PopulateDynamicTitleToken(1, "Name")
	decorReport:AddDataCollection(collector.FromDataPackage, "ID")
	decorReport:AddDataCollection(collector.OpenEndedQuestion, "What was the issue with this Decor?")
	decorReport:RegisterPopEvent(event.Tooltip, tooltips.housingDecor)

	local roomReport = PTR_IssueReporter.CreateSurvey(23, "Issue Report: %s")
	PTR_IssueReporter.AttachDefaultCollectionToSurvey(roomReport, true)
	roomReport:PopulateDynamicTitleToken(1, "Name")
	roomReport:AddDataCollection(collector.FromDataPackage, "ID")
	roomReport:AddDataCollection(collector.OpenEndedQuestion, "What was the issue with this Room?")
	roomReport:RegisterPopEvent(event.Tooltip, tooltips.housingRoom)

	local componentReport = PTR_IssueReporter.CreateSurvey(24, "Issue Report: %s")
	PTR_IssueReporter.AttachDefaultCollectionToSurvey(componentReport, true)
	componentReport:PopulateDynamicTitleToken(1, "Name")
	componentReport:AddDataCollection(collector.FromDataPackage, "ID")
