...
function ClassTalentSearchMixin:InitializeSearch()
	...
	if self.missingFromActionBarSuggested then
...
function ClassTalentSearchMixin:SetPreviewResultSearch(previewSearchText)
	...
	if previewResults and #previewResults > 0 then
		previewResults = self:TransformPreviewResults(previewResults);
	end
...
-- Can be overridden for additional/custom filtering.
function ClassTalentSearchMixin:TransformPreviewResults(previewResults)
...
function ClassTalentSearchMixin:DisplayFullSearchResults()
	...
	if self.HeroTalentsContainer then
