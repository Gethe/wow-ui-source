...
local function SetSortingByUnitPrice(value)
	SetCVar("auctionSortByUnitPrice", value);
end

local function IsSortingByBuyoutPrice()
	return GetCVarBool("auctionSortByBuyoutPrice");
end

local function SetSortingByBuyoutPrice(value)
	SetCVar("auctionSortByBuyoutPrice", value);
end

local function PriceSortHeaderName()
	if (IsSortingByUnitPrice()) then
		if (IsSortingByBuyoutPrice()) then
			return AUCTION_SORT_HEADER_UNIT_BUYOUT_PRICE;
		else
			return AUCTION_SORT_HEADER_UNIT_BID_PRICE;
		end
	else
		if (IsSortingByBuyoutPrice()) then
			return AUCTION_SORT_HEADER_TOTAL_BUYOUT_PRICE;
		else
			return AUCTION_SORT_HEADER_TOTAL_BID_PRICE;
		end
	end
end

local function PriceSortName()
	if (IsSortingByUnitPrice()) then
		if (IsSortingByBuyoutPrice()) then
			return "unitprice";
		else
...
function BrowsePriceOptionsButton_OnClick()
	if (BrowsePriceOptionsFrame:IsShown()) then
		BrowsePriceOptionsFrame:Hide();
	else
		BrowsePriceOptionsFrame:Show();
	end
end

function BrowsePriceOptionsFrame_OnShow()
	SortByBidPriceButton:SetChecked(not IsSortingByBuyoutPrice());
	SortByBuyoutPriceButton:SetChecked(IsSortingByBuyoutPrice());
	SortByTotalPriceButton:SetChecked(not IsSortingByUnitPrice());
	SortByUnitPriceButton:SetChecked(IsSortingByUnitPrice());
end

function BrowsePriceOptionsRadioButton_OnClick(buttonID)
	local oldSortByBuyoutPrice = IsSortingByBuyoutPrice();
	local oldSortByUnitPrice = IsSortingByUnitPrice();

	if (buttonID == 1) then
		-- Current Bid
		SortByBidPriceButton:SetChecked(true);
		SortByBuyoutPriceButton:SetChecked(false);
		SetSortingByBuyoutPrice("0");
	elseif (buttonID == 2) then
		-- Buyout
		SortByBidPriceButton:SetChecked(false);
		SortByBuyoutPriceButton:SetChecked(true);
		SetSortingByBuyoutPrice("1");
	elseif (buttonID == 3) then
		-- Total Price
		SortByTotalPriceButton:SetChecked(true);
		SortByUnitPriceButton:SetChecked(false);
		SetSortingByUnitPrice("0");
	elseif (buttonID == 4) then
		-- Unit Price
		SortByTotalPriceButton:SetChecked(false);
		SortByUnitPriceButton:SetChecked(true);
		SetSortingByUnitPrice("1");
	else
		error("unknown buttonID "..buttonID);
	end

	if (IsSortingByBuyoutPrice() ~= oldSortByBuyoutPrice or IsSortingByUnitPrice() ~= oldSortByUnitPrice) then
		BrowseCurrentBidSort:SetText(PriceSortHeaderName());

		-- If currently sorting by the price column, re-sort with the new options,
		-- otherwise just redraw the current items with the new price options.
		local existingSortColumn, existingSortReverse = GetAuctionSortColumn("list");
		if (existingSortColumn == "bid") then
			AuctionFrame_SetSort("list", existingSortColumn, existingSortReverse);
...
function AuctionFrame_SetSort(sortTable, sortColumn, oppositeOrder)
	...
	for index, row in pairs(AuctionSort[sortTable.."_"..sortColumn]) do
		...
		if (oppositeOrder) then
			SortAuctionSetSort(sortTable, sort, not row.reverse);
		else
			SortAuctionSetSort(sortTable, sort, row.reverse);
		end
	end
end
...
function BrowseSearchButton_OnUpdate(self, elapsed)
	if ( CanSendAuctionQuery("list") ) then
		...
		BrowsePriceOptionsButtonFrame.Button:Enable();
		AuctionFrameBrowse_UpdateArrows();
	else
		self:Disable();
		BrowsePrevPageButton:Disable();
		...
		BrowsePriceOptionsButtonFrame.Button:Disable();
	end
...
function AuctionFrameFilter_OnClick(self, button)
	if ( self.type == "category" ) then
		...
		if (AuctionFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", AuctionFrameBrowse.selectedCategoryIndex)) then
			...
			BrowsePriceOptionsButtonFrame:Show();
...
function AuctionFrameBrowse_Update()
	if (not AuctionFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", AuctionFrameBrowse.selectedCategoryIndex)) then
		...
		-- Update price sort column header based on CVars
		BrowseCurrentBidSort:SetText(PriceSortHeaderName());
	...
	if ( event == "NEW_AUCTION_UPDATE") then
		...
		if (C_WowTokenPublic.IsAuctionableWowToken(itemID)) then
			...
			if (texture) then
				...
				AuctionsItemButton:ClearNormalTexture();
			end
...
local function SetupUnitPriceTooltip(tooltip, type, auctionItem, insertNewline)
	if ( auctionItem and auctionItem.itemCount > 1 ) then
		...
		local showTooltipPricePerUnit = not (type == "list" and IsSortingByUnitPrice());
		...
		if ( hasBid ) then
			...
			amount = auctionItem.bidAmount;
			if ( showTooltipPricePerUnit ) then
				prefix = AUCTION_TOOLTIP_BID_PREFIX;
...
function AuctionFrameItem_OnLeave(self)
	GameTooltip_Hide();
	ResetCursor();
