...
local AUTOCOMPLETE_LIST = AUTOCOMPLETE_LIST;
	...
	AUTOCOMPLETE_LIST.HOUSE_FINDER		= AUTOCOMPLETE_LIST_TEMPLATES.ALL_BNET;
...
function AutoComplete_UpdateResults(self, results, context)
	...
	for i=1, numReturns do
		...
		local displayName = Ambiguate and Ambiguate(results[i].name, context or "all") or results[i].name;
		local displayText;
...
function AutoCompleteButton_OnClick(self)
	...
	local editBoxText = editBox:GetText();
	local name = Ambiguate and Ambiguate(self.nameInfo.name, "none") or self.nameInfo.name;
	local newText;
