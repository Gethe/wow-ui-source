...
function ExhaustionTickMixin:UpdateTickPosition()
	...
	if ( not exhaustionThreshold or exhaustionThreshold <= 0 or GameRulesUtil.IsPlayerAtEffectiveMaxLevel() or IsXPUserDisabled() ) then
		...
		local exhaustionTickSet = max(widthRatio * parentWidth, 0);
		self:ClearAllPoints();

		if ( exhaustionTickSet > parentWidth and not ShouldRestedXpBarDisplayWhenOverflowing() ) then
			self:GetParent().ExhaustionLevelFillBar:Hide();
		else
			-- Clamp so rested XP overflowing into the next level still fills the bar to its edge instead of vanishing
			exhaustionTickSet = Clamp(exhaustionTickSet, 0, parentWidth);
			self:SetPoint("CENTER", self:GetParent(), "LEFT", exhaustionTickSet, EXHAUSTION_TICK_OFFSET_Y or 0);
			self:GetParent().ExhaustionLevelFillBar:Show();
			self:GetParent().ExhaustionLevelFillBar:SetWidth(exhaustionTickSet);
			self:GetParent().ExhaustionLevelFillBar:SetTexCoord(0, min(widthRatio, 1), 0, 1);
		end
	end
end
