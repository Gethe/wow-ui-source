...
local function GetFollowerDungeonFailureMessage()
	-- Get a list of follower dungeons
	local followerDungeons = {};
	for _, dungeonID in ipairs(GetLFDChoiceOrder()) do
		if dungeonID >= 0 and C_LFGInfo.IsLFGFollowerDungeon(dungeonID) then
			table.insert(followerDungeons, dungeonID);
		end
	end

	-- Find the first failure message and store it, but continue processing the list building up a final list of any valid follower dungeons
	local firstDeclinedMessage;
	local hasAnyAvailableFollowerDungeons;
	for index, followerDungeonID in ipairs(followerDungeons) do	
		local declinedMessage = LFGConstructDeclinedMessage(followerDungeonID);
		if declinedMessage and declinedMessage ~= "" then
			firstDeclinedMessage = firstDeclinedMessage or declinedMessage;
		else
			hasAnyAvailableFollowerDungeons = true;
		end
	end

	if not hasAnyAvailableFollowerDungeons then
		return firstDeclinedMessage;
	end
end
