...
function FilterDropdownMinRatingEditBoxMixin:OnTextChanged()
	self:GetParent():OnMinRatingChanged();
end

function FilterDropdownMinRatingEditBoxMixin:OnLoad()
	local enabled = C_LFGList.GetAdvancedFilter();
	self:SetNumber(enabled.minimumRating);
end

FilterDropdownMinRatingFrameMixin = {};

function FilterDropdownMinRatingFrameMixin:SetMinRatingChangedCallback(callback)
	self.minRatingChangeCallback = callback;
end

function FilterDropdownMinRatingFrameMixin:OnMinRatingChanged()
	if self.minRatingChangeCallback then
		self:minRatingChangeCallback();
	end
end

function LFGListAdvancedFiltersReset()
	local enabled = C_LFGList.GetAdvancedFilter();
...
function LFGListUTil_InitializeAdvancedFilter(dropdown)
	local entry = UIDropDownMenu_CreateInfo();
	...
	header.dist = 0;
	header.isTitle = true;
	header.isUninteractable = true;
	header.notCheckable = true;

	local shouldShowLanguages = LFGListCanChangeLanguages();
	...
	if IsPlayerAtEffectiveMaxLevel() then
		...
		AddHeader(LFG_LIST_REQUIRE);
		--only show the roles that apply to your class.
		local availTank, availHealer, availDPS = C_LFGList.GetAvailableRoles();
		if availTank then
			...
			local function AddGroup(groups)
				for _, activityId in ipairs(groups) do
					...
					if name then
						...
						entry.checked = activitySet[activityId];
						entry.func = function(self,_,_,checked) 
							ActivitiesListToSet();
		...
		UIDropDownMenu_AddSpace();
		AddHeader(LFG_LIST_MINIMUM_RATING)
		local info = UIDropDownMenu_CreateInfo();
		info.customFrame = MinRatingFrame;
		...
		if shouldShowLanguages then
			UIDropDownMenu_AddSpace();
			AddHeader(LANGUAGES_LABEL)
		end
	end
	if shouldShowLanguages then
