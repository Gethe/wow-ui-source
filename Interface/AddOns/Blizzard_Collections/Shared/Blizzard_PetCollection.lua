...
function PetJournalHealPetSpellFrameMixin:IsLocked()
	return C_PetBattles.IsInBattle() or not C_Spell.IsSpellUsable(self.spellID);
end
...
function PetJournalHealPetSpellFrameMixin:OnSetTooltip(tooltip)
	...
	elseif C_PetBattles.IsInBattle() then
		GameTooltip_AddErrorLine(tooltip, PET_JOURNAL_HEAL_SPELL_IN_BATTLE);
	end
end
...
function PetJournalSummonRandomPetSpellFrameMixin:OnIconClick()
	local hasFavoritePets = C_PetJournal.HasFavoritePets();
	C_PetJournal.SummonRandomPet(hasFavoritePets);
end

function PetJournalSummonRandomPetSpellFrameMixin:OnIconDragStart()
	C_PetJournal.PickupSummonRandomPet();
end

function PetJournalSummonRandomPetSpellFrameMixin:UpdateCooldown()
...
function PetJournalSummonRandomPetSpellFrameMixin:IsAvailable()
	local numPets, numOwned = C_PetJournal.GetNumPets();
	return numOwned > 0;
end

function PetJournalSummonRandomPetSpellFrameMixin:IsLocked()
...
function PetJournal_UpdatePetLoadOut(forceSceneChange)
	...
	for i=1,MAX_ACTIVE_PETS do
		...
		elseif (petID == nil or speciesID == nil) then
			...
			local colorData = ColorManager.GetColorDataForItemQuality(rarity-1);
			if colorData then
				loadoutPlate.qualityBorder:SetVertexColor(colorData.r, colorData.g, colorData.b);
			end
...
function PetJournal_InitPetButton(pet, elementData)
	...
	if isOwned then
		...
		local colorData = ColorManager.GetColorDataForItemQuality(rarity-1);
		if colorData then
			pet.iconBorder:SetVertexColor(colorData.r, colorData.g, colorData.b);
		end
...
function PetJournal_UpdatePetCard(self, forceSceneChange)
	...
	if PetJournalPetCard.petID then
		...
		if ( canBattle ) then
			...
			local colorData = ColorManager.GetColorDataForItemQuality(rarity-1);
			if colorData then
				self.QualityFrame.quality:SetVertexColor(colorData.r, colorData.g, colorData.b);
				self.PetInfo.qualityBorder:SetVertexColor(colorData.r, colorData.g, colorData.b);
			end
		else
			self.QualityFrame:Hide();
			self.PetInfo.qualityBorder:Hide();
