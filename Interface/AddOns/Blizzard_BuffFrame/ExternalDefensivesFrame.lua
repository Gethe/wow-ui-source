...
function ExternalDefensivesFrameMixin:UpdateAuras()
	...
	AuraUtil.ForEachAura(PlayerFrame.unit, "HELPFUL|EXTERNAL_DEFENSIVE", self.maxAuras, function(auraData)
		...
		self.auraInfo[auraInfoIndex] = {index = auraIndex, texture = auraData.icon, count = auraData.applications, debuffType = auraData.dispelName, duration = auraData.duration,  expirationTime = auraData.expirationTime, timeMod = auraData.timeMod, hideUnlessExpanded = hideUnlessExpanded, auraType = "Buff" };
