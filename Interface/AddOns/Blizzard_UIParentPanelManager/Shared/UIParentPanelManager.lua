...
function AreAllPanelsDisallowed()
	...
	if not currentWindow then
		currentWindow = GetUIPanel("fullscreen");
		if not currentWindow then
			return false;
		end
	end
...
function ValidateFramePosition(frame, offscreenPadding, returnOffscreen)
	...
	if ( bottom < (0 + MainActionBar:GetHeight() + offscreenPadding)) then
		-- Off the bottom of the screen
		newAnchorY = MainActionBar:GetHeight() + frame:GetHeight() - GetScreenHeight();
