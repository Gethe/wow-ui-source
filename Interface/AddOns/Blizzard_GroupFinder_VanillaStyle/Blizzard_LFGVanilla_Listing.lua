...
function LFGListingCategorySelection_AddButton(self, btnIndex, categoryID)
	...
	local atlasName = (LFGLISTING_CATEGORY_TEXTURES[categoryID] or LFGLISTING_CATEGORY_TEXTURE_DEFAULT);
...
function LFGListingActivityView_OnShow(self)
	...
	if (categoryInfo.autoChooseActivity) then
		...
		if self.BarComponents then
			for i, component in ipairs(self.BarComponents) do
				component:Hide();
			end
		end
		self.ScrollBox:Hide();
