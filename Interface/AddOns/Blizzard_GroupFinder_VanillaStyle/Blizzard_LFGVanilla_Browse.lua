...
function LFGBrowseMixin:OnLoad()
	...
	self:RegisterEvent("LFG_LIST_SHOW_SEARCH");
	...
	local padding = 0;
	local ySpacing = 0;
	local myScrollBox = self.ScrollBox;
	local myScrollBar = self.ScrollBar;
	local view = CreateScrollBoxListTreeListView(indent, padding, padding, padding, padding, ySpacing);
	view:SetElementFactory(function(factory, elementData)
		local data = elementData.data;
		if data.dividerType then
			factory("LFGBrowseSearchEntryGroupingTemplate", function(frame, elementNode)
				if data.dividerType == LFGVanillaBrowseDividerType.CategorySolo then
					frame.CategoryLabel:SetText(LFG_LIST_CATEGORY_SOLO_PLAYERS);
				elseif data.dividerType == LFGVanillaBrowseDividerType.CategoryGroup then
					frame.CategoryLabel:SetText(LFG_LIST_CATEGORY_GROUPS);
				else
					frame.CategoryLabel:Hide();
				end

				frame:SetScript("OnClick", function(button, buttonName)
					elementNode:ToggleCollapsed(self.setCollapsedChildren, TreeDataProviderConstants.DoInvalidation);
					if elementNode.collapsed then
	...
	ScrollUtil.InitScrollBoxListWithScrollBar(myScrollBox, myScrollBar, view);

	local scrollBoxAnchorsWithBar = {
		CreateAnchor("TOPLEFT", myScrollBox.AnchorOffsetLeft, myScrollBox.AnchorOffsetTop),
		CreateAnchor("BOTTOMRIGHT", myScrollBox.AnchorOffsetRight - myScrollBox.ScrollBarWidth, myScrollBox.AnchorOffsetBottom);
	};
	local scrollBoxAnchorsWithoutBar = {
		scrollBoxAnchorsWithBar[1],
		CreateAnchor("BOTTOMRIGHT", myScrollBox.AnchorOffsetRight, myScrollBox.AnchorOffsetBottom);
	};
	ScrollUtil.AddManagedScrollBarVisibilityBehavior(myScrollBox, myScrollBar, scrollBoxAnchorsWithBar, scrollBoxAnchorsWithoutBar);
	...
	self.selectionBehavior = ScrollUtil.AddSelectionBehavior(myScrollBox, SelectionBehaviorFlags.Deselectable);
...
function LFGBrowseMixin:UpdateResults()
	...
	if ( self.searching ) then
		...
		if(self.totalResults == 0 or self.searchFailed) then
			...
			local currentSubtree = nil;
			local currentCategory = nil;
			for index = 1, #results do
				local resultID = results[index];
				local searchResultInfo = C_LFGList.GetSearchResultInfo(resultID);
				local hasSelf = searchResultInfo.hasSelf;
				local numMembers = searchResultInfo.numMembers;
				local dividerCategory = nil;
				if not hasSelf then
					if numMembers <= 1 then
						dividerCategory = LFGVanillaBrowseDividerType.CategorySolo;
					else
						dividerCategory = LFGVanillaBrowseDividerType.CategoryGroup;
					end
				end
				if LFGVANILLA_SETTING_BROWSE_SHOW_COLLAPSIBLE_CATEGORIES and dividerCategory and dividerCategory ~= currentCategory then
...
function LFGBrowseMixin:SearchActiveEntry()
	...
	if (activeEntryInfo) then
		self:SetSelectedSearchActivities(activeEntryInfo.activityIDs);
	end

	LFGBrowse_DoSearch();
end

function LFGBrowseMixin:ShowSearchForActivities(activityIDs)
	LFGVanilla_ShowFrame(LFGPARENT_BROWSING_TAB_INDEX);

	if (activityIDs) then
		self:SetSelectedSearchActivities(activityIDs);
	end

	LFGBrowse_DoSearch();
end

function LFGBrowseMixin:SetSelectedSearchActivities(activityIDs)
	if (not self.CategoryDropdown or not self.ActivityDropdown) then
		return;
	end
...
function LFGBrowseMixin:ValidateSelection()
	local selectedResultID = self.selectionBehavior:HasSelection() and LFGBrowseFrame.selectionBehavior:GetSelectedElementData()[1].data.resultID or nil;
...
function LFGBrowseSearchEntry_Update(self)
	...
	if (isSolo) then
		...
		self.Name:SetPoint("TOPLEFT", self.PartyIcon, "TOPLEFT", self.NameSoloOffsetX, self.NameSoloOffsetY);
	...
	LFGBrowseGroupDataDisplay_Update(self.DataDisplay, displayType, maxNumPlayers, displayData, searchResultInfo.isDelisted, isSolo, soloRoles, searchResultInfo.comment, searchResultInfo.hasSelf);
...
function LFGBrowseSearchEntry_SetSelection(self, selected)
	...
	if (selected) then
		self.Highlight:Hide();
	elseif (InputUtil.IsMouseOver(self)) then
		self.Highlight:Show();
	end
end
...
----------Group Data Display
-------------------------------------------------------
function LFGBrowseGroupDataDisplay_Update(self, displayType, maxNumPlayers, displayData, disabled, isSolo, soloRoles, comment, hasSelf)
	...
	if (self.showDelistButton and hasSelf and (not IsInGroup() or UnitIsGroupLeader("player"))) then
		self.DelistButton:Show();
	end
end
...
function LFGBrowseActivityDropdownMixin:UpdateHeader()
	...
	elseif #self.selectedValues == 1 then
		...
		self.ResetButton:SetShown(self.showResetButton);
	else
...
function LFGBrowseSendMessageButtonMixin:OnClick(button)
	...
	if (selectedElement) then
		local searchResultInfo = C_LFGList.GetSearchResultInfo(selectedElement.data.resultID);
	...
	if (resultID) then
		...
		if (searchResultInfo) then
			...
			if (resultIsEligible and playerCanInvite) then
				...
				inviteFunc = function() C_PartyInfo.InviteUnit(searchResultInfo.leaderName); end;
			end
		end
	end
