...
function GraphicsOverrides.CreateHiResOptions(category, layout)
	...
	local setting = Settings.RegisterProxySetting(category, "PROXY_HIGH_RES_TEXTURES", Settings.DefaultVarLocation,
	...
	if not C_BattleNet.AreHighResTexturesInstalled() then
		...
		local initializer = CreateSettingsButtonInitializer(OPTION_HD_TEXTURES, HD_TEXTURES_BUTTON, OnClick, OPTION_TOOLTIP_HD_TEXTURES, addSearchTags);
		initializer.hideText = true;

		local version = GetBuildInfo();
		initializer.showNew = version == "4.4.0";
		initializer:AddShownPredicate(BNConnected);
