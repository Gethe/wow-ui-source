...
function SettingsPanelMixin:FindInitializersMatchingSearchText(searchText)
	...
	local function ParseCategory(category, parentCategory)
		...
		if layout:IsVerticalLayout() then
			for _, initializer in layout:EnumerateInitializers() do
				if not initializer:IsSearchIgnoredInLayout(layout) and initializer:ShouldShow() then
					local score = initializer:MatchesSearchTags(words);
					if score then
						if not scoredInitializers[score] then
							scoredInitializers[score] = {};
						end

						table.insert(scoredInitializers[score], { initializer = initializer, category = redirectCategory });
					end
				end
			end
		end
	...
	for _, score in ipairs(matchScores) do
		for _, match in ipairs(scoredInitializers[score]) do
			local category = match.category;

			local list = scoredInitializersList[category];
			if not list then
				list = {};
				scoredInitializersList[category] = list;
				table.insert(initializerLists, list);
			end

			if not found[category] then
				found[category] = true;
				table.insert(list, CreateSettingsListSearchCategoryInitializer(category));
			end

			table.insert(list, match.initializer);
		end
	end

	-- Flatten the category lists back into a single array.
	local initializers = {};
	for index, categoryList in ipairs(initializerLists) do
		tAppendAll(initializers, categoryList);
	end
...
function SettingsPanelMixin:OnSearchTextChanged()
	...
	elseif text and string.len(text) > 0 then
		...
		for _, initializer in ipairs(initializers) do
			...
			if parentInitializer and not added[parentInitializer] then
				added[parentInitializer] = true;
				layout:AddInitializer(parentInitializer);
			end

			if not added[initializer] then
...
function SettingsPanelMixin:GetCurrentLayout()
