...
function ActionBarMixin:SetShowGrid(showGrid, reason)
	...
	self:UpdateFrameStrata(showGrid, reason);
    self:UpdateShownButtons();
...
function ActionBarMixin:ShouldRaise(showGrid, reason)
	if showGrid and (reason == ACTION_BUTTON_SHOW_GRID_REASON_EVENT) then
		...
		if cursorType then
			return cursorTypesThatRaiseActionBar[cursorType] ~= nil;
		end
	end
...
function ActionBarMixin:UpdateFrameStrata(showGrid, reason)
	self:SetFrameStrata(self:ShouldRaise(showGrid, reason) and "TOOLTIP" or "MEDIUM");
end
