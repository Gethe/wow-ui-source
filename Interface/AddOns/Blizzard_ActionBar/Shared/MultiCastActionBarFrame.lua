...
local MULTI_CAST_BUTTON_SIZE = 30;
...
function MultiCastActionButton_OnLoad(self)
	...
	normalTexture:SetSize(MULTI_CAST_BUTTON_SIZE, MULTI_CAST_BUTTON_SIZE);
	self.HighlightTexture:SetSize(MULTI_CAST_BUTTON_SIZE, MULTI_CAST_BUTTON_SIZE);
	self.CheckedTexture:SetSize(MULTI_CAST_BUTTON_SIZE, MULTI_CAST_BUTTON_SIZE);
	self.PushedTexture:SetSize(MULTI_CAST_BUTTON_SIZE, MULTI_CAST_BUTTON_SIZE);
	self.Arrow:Hide();
...
function MultiCastActionButton_OnEvent(self, event, ...)
	ActionBarActionButtonDerivedMixin.ActionBarActionButtonDerivedMixin_OnEvent(self, event, ...);
	if ( event == "MODIFIER_STATE_CHANGED" ) then
		if ( IsModifiedClick("SHOWMULTICASTFLYOUT") and self:IsMouseOver() ) then
