...
function PetStable_Update(updateModel)
	...
	for i=1, NUM_PET_ACTIVE_SLOTS do
 	...
 	if ( PetStableFrame.selectedPet ) then
		...
		local foodTypes = C_StableInfo.GetStablePetFoodTypes(PetStableFrame.selectedPet);
		if #foodTypes > 0 then
			PetStableDiet.tooltip = format(PET_DIET_TEMPLATE, table.concat(foodTypes, PET_FOOD_DELIMIT));
			PetStableDiet:Show();
		else
			PetStableDiet:Hide();
		end
