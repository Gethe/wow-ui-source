	...
	for i=1, NUM_FACTIONS_DISPLAYED, 1 do
		...
		if ( factionIndex <= numFactions ) then
			...
			if ( isHeader ) then
				...
				local factionStanding = GetText("FACTION_STANDING_LABEL"..standingID, gender);
				local factionName = _G["ReputationBar"..i.."FactionName"];
				factionName:SetText(name);
				_G["ReputationBar"..i.."FactionStanding"]:SetText(factionStanding);
				
				local atWarIndicator = _G["ReputationBar"..i.."AtWarCheck"];
				local rightBarTexture = _G["ReputationBar"..i.."ReputationBarRight"];
