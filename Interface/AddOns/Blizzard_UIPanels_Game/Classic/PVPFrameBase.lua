...
function PVPTeam_SoloUpdate()
	...
	for index=1, MAX_ARENA_TEAMS do
		...
		-- Pull Values
		rating, _, _, seasonPlayed, seasonWins, weekPlayed, weekWins, _, _, _, rank, teamSize = GetPersonalRatedInfo(index);
		...
		if (rating > 0 or seasonPlayed > 0) then
			...
			if (rank > 0) then
				getglobal(standard.."Rank"):SetText(rank);
				getglobal(standard.."Rank"):SetVertexColor(rankColor.r, rankColor.g, rankColor.b);
			else
			...
			standardBanner:SetTexture("Interface\\PVPFrame\\PVP-Banner-"..teamSize);
...
function PVPTeam_OnMouseDown(self)
	if ( GetCurrentArenaSeasonUsesTeams() and GetArenaTeam(self:GetID()) ) then
