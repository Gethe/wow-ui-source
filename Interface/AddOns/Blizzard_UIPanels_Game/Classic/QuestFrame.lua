...
function GetQuestFrameSize()
	local expansionToUse = GetClassicExpansionLevel();
	-- Need to fill in the size for the new expansion in QUEST_FRAME_DIMENSIONS above
	if expansionToUse > LE_EXPANSION_MISTS_OF_PANDARIA then
		expansionToUse = LE_EXPANSION_MISTS_OF_PANDARIA;
	end
	return QUEST_FRAME_DIMENSIONS[expansionToUse];
end
...
function QuestRewardCompleteButton_OnClick()
	...
	if ( QuestInfoFrame.itemChoice == 0 and GetNumQuestChoices() > 0 ) then
		...
		local tbcAndBeyond = ClassicExpansionAtLeast(LE_EXPANSION_BURNING_CRUSADE);
...
function QuestFrameProgressItems_Update()
	...
	if ( numRequiredItems > 0 or GetQuestMoneyToGet() > 0 or numRequiredCurrencies > 0) then
		...
		local tbcAndBeyond = ClassicExpansionAtLeast(LE_EXPANSION_BURNING_CRUSADE);
		if (actualNumRequiredItems + numRequiredCurrencies > 0 or (tbcAndBeyond and GetQuestMoneyToGet() > 0)) then
...
function QuestFrameItems_Update(questState)
	...
	if ( numQuestSpellRewards > 0 ) then
		...
		elseif tbcAndBeyond and not spellInfo.isSpellLearned then
