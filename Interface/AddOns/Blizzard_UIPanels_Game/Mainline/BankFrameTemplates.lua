...
local DEFAULT_MAXIMUM_TOTAL_SLOTS_PER_PAGE = 96; -- Only used if ShouldUsePlayerBagsInBank is true

BankFrameBaseMixin = CreateFromMixins(CallbackRegistryMixin);

BankFrameBaseMixin:GenerateCallbackEvents(
{
	"TitleUpdateRequested",
});

function BankFrameBaseMixin:OnLoad()
	CallbackRegistryMixin.OnLoad(self);
	self:AddDynamicEventMethod(self, BankFrameBaseMixin.Event.TitleUpdateRequested, self.OnTitleUpdateRequested);
...
function BankFrameBaseMixin:OnTitleUpdateRequested(titleText)
	self:SetTitle(titleText);
end

function BankFrameBaseMixin:InitializeTabSystem()
	if(self.TabSystem) then
...
function BankFrameBaseMixin:SetTabIDToBankType(tabID, bankType)
	self.TabIDToBankType[tabID] = bankType;
end

function BankFrameBaseMixin:GenerateBankTypeTabs()
	...
	if(self.TabSystem) then
		...
		self.characterBankTabID = 1;
		self.accountBankTabID = 2;
	end

	self:SetTabIDToBankType(self.characterBankTabID, Enum.BankType.Character);
	self:SetTabIDToBankType(self.accountBankTabID, Enum.BankType.Account);
end

function BankFrameBaseMixin:SetTab(tabID)
	self.BankPanel:SetBankType(self.TabIDToBankType[tabID]);
	self.BankPanel.currentPage = 1;
	if(self.TabSystem) then
...
function BankFrameBaseMixin:UpdateWidthForSelectedTab()
	local tabPage = self:GetElementsForTab(self:GetTab())[1];
	self:SetWidth(tabPage:GetWidth());
	UpdateUIPanelPositions(self);
end

function BankFrameBaseMixin:RefreshTabVisibility()
	if(self.TabSystem) then
...
function BankFrameBaseMixin:SelectDefaultTab()
	self:SelectFirstAvailableTab();
end

function BankFrameBaseMixin:SelectFirstAvailableTab()
	if(self.TabSystem) then
		for _index, tabID in ipairs(self:GetTabSet()) do
...
function BankFrameBaseMixin:GetActiveBankType()
	return self.BankPanel:IsShown() and self.BankPanel:GetActiveBankType() or nil;
end

function BankFrameBaseMixin:OnShow()
...
function BankFrameBaseMixin:OnHide()
...
function BankUtil_IsAccountBankDepositRefundable(itemLocation)
	if not itemLocation or not itemLocation.bagID or not itemLocation:IsValid() then
		return false;
	end
...
function BankPanelItemButtonMixin:UpdateBackgroundForBankType()
	...
	if self.bankType == Enum.BankType.Account then
		...
		local characterBankBackgroundAtlas = self.characterBankBackgroundAtlas or "bags-item-slot64";
		self.Background:SetAtlas(characterBankBackgroundAtlas, TextureKitConstants.IgnoreAtlasSize);
	end
end
...
function BankPanelMixin:OnPageSwitched(id)
	local newPage = self.currentPage;
	if (id == 1) then
	...
	local totalSlots = self:GetTotalSlotsInAllTabs();
	local maximumTotalSlotsPerPage = self:GetMaximumTotalSlotsPerPage();
	local maxPages = math.ceil(totalSlots / maximumTotalSlotsPerPage);
...
function BankPanelMixin:OnPageSelected(pageNumber)
	...
	local newPage = math.max(1, math.min(pageNumber, maxPages));
	self.currentPage = newPage;
	self:GenerateItemSlotsForSelectedTab();
end
...
function BankPanelMixin:OnLoad()
	...
	EventRegistry:RegisterCallback("BankPanelMixin.PageSwitched", self.OnPageSwitched, self);
	EventRegistry:RegisterCallback("BankPanelMixin.PageSelected", self.OnPageSelected, self);
	...
	local itemButtonTemplate = self.itemButtonTemplate or "BankItemButtonTemplate";
	self.itemButtonPool = CreateFramePool("ItemButton", self, itemButtonTemplate, BankItemButtonResetter);
...
function BankPanelMixin:OnEvent(event, ...)
	...
	elseif event == "BAG_UPDATE" then
		local containerID = ...;
		if self.selectedTabID == containerID or C_Bank.ShouldUsePlayerBagsInBank() then
			self:MarkDirty();
		end
	...
	elseif event == "ITEM_LOCK_CHANGED" then
		...
		if not itemInSelectedTab and not C_Bank.ShouldUsePlayerBagsInBank() then
			return;
		end

		local itemButton = self:FindItemButtonByBankTabAndContainerSlotID(bankTabID, containerSlotID);
		if itemButton then
			itemButton:Refresh();
		end
...
function BankPanelMixin:ShouldShowPurchasePrompt()
	return self.PurchaseTab and self.PurchaseTab:IsSelected() and C_Bank.CanPurchaseBankTab(self.bankType);
end

function BankPanelMixin:ShowPurchasePrompt()
	if(C_Bank.ShouldUsePlayerBagsInBank() and self:GetActiveBankType() == Enum.BankType.Character) then
		return;
	end
	self.TabSettingsMenu:Hide();
	self:SetHeaderEnabled(false);
...
function BankPanelMixin:RequestTitleRefresh()
	...
	self:GetBankContainerFrame():TriggerEvent(BankFrameBaseMixin.Event.TitleUpdateRequested, bestTitleForBankType);
end
...
function BankPanelMixin:GenerateAndPlaceOnePageOfItemButtons(numColumns, totalSlots, bagID, isFirstButton, currentColumn, slotIDOffset, firstButtonXOffset, ignorePages, lastRowStarterButton, lastCreatedButton)
	local maximumTotalSlotsPerPage = self:GetMaximumTotalSlotsPerPage();
	local neededOffsetToReachPage = (self.currentPage-1) * maximumTotalSlotsPerPage;
	local maximumOffsetForPage = neededOffsetToReachPage + maximumTotalSlotsPerPage;
	local useUniformItemSlotSpacing = self.useUniformItemSlotSpacing;
	local numSubColumns = 2;
	for containerSlotID = slotIDOffset + 1, slotIDOffset + totalSlots do
		if (ignorePages or (containerSlotID > neededOffsetToReachPage and containerSlotID <= maximumOffsetForPage)) then
			local button = self.itemButtonPool:Acquire();
				
			local needNewRow = (containerSlotID % numColumns) == 1;
			if isFirstButton then
				local xOffset, yOffset = firstButtonXOffset, -63;
			...
			elseif needNewRow then
				...
				if not useUniformItemSlotSpacing then
			...
			button:Init(self.bankType, bagID, containerSlotID - slotIDOffset);
			button:Show();
	...
	return isFirstButton, currentColumn, lastRowStarterButton, lastCreatedButton;
end

function BankPanelMixin:GetTotalSlotsInAllTabs()
	local totalSlots = 0;
	for index, bankTabData in ipairs(self.purchasedBankTabData) do
		totalSlots = totalSlots + C_Container.GetContainerNumSlots(bankTabData.ID);
	end
	return totalSlots;
end

function BankPanelMixin:GetMaximumTotalSlotsPerPage()
	return self.maximumTotalSlotsPerPage or DEFAULT_MAXIMUM_TOTAL_SLOTS_PER_PAGE;
end
...
function BankPanelMixin:GenerateItemSlotsForSelectedTab()
	...
	if not self.selectedTabID or self.selectedTabID == PURCHASE_TAB_ID then
		EventRegistry:TriggerEvent("BankPanelMixin.PageInfoChanged", 1);
		return;
	end

	if (C_Bank.ShouldUsePlayerBagsInBank()) then
		local totalSlots = self:GetTotalSlotsInAllTabs();
		local totalDisplayedSlots = totalSlots;
		local maximumTotalSlotsPerPage = self:GetMaximumTotalSlotsPerPage();

		local slotPageRatio = totalSlots / maximumTotalSlotsPerPage;
		local numPagesNeeded = math.ceil(slotPageRatio);
		if(numPagesNeeded < self.currentPage) then
			self.currentPage = numPagesNeeded;
		end
		if(self.currentPage < 1) then
			self.currentPage = 1;
		end
		-- Next we grab the number of slots we're actually gonna display for this current page
		if(totalSlots > maximumTotalSlotsPerPage) then
			if(numPagesNeeded == self.currentPage and slotPageRatio ~= self.currentPage) then
				totalDisplayedSlots = totalSlots % (maximumTotalSlotsPerPage);
			else
				totalDisplayedSlots = maximumTotalSlotsPerPage;
			end
		end
		local numColumns = 8;
		-- Find our desired number of rows, rounding up
		local numDisplayedRows = math.ceil(totalDisplayedSlots / numColumns);

		-- We may need to resize the bank window
		local addedHeight = (numDisplayedRows - 6) * 47;
		if(addedHeight < 0) then
			addedHeight = 0;
		end
		local newBankFrameHeight = 460 + addedHeight;
		self:GetParent():SetSize(self:GetWidth(), newBankFrameHeight);
		self:SetSize(self:GetWidth(), newBankFrameHeight);
		EventRegistry:TriggerEvent("BankPanelMixin.PageInfoChanged", self.currentPage);

		local isFirstButton = true;
		local lastRowStarterButton;
		local lastCreatedButton;
		local currentColumn = 1;
		local slotIDOffset = 0;
		local firstButtonXOffset = 47;
		local ignorePages = false;
		for index, bankTabData in ipairs(self.purchasedBankTabData) do
			local numSlots = C_Container.GetContainerNumSlots(bankTabData.ID);
			isFirstButton, currentColumn, lastRowStarterButton, lastCreatedButton = self:GenerateAndPlaceOnePageOfItemButtons(numColumns, numSlots, bankTabData.ID, isFirstButton, currentColumn, slotIDOffset, firstButtonXOffset, ignorePages, lastRowStarterButton, lastCreatedButton);
			slotIDOffset = slotIDOffset + numSlots;
		end
	else
		EventRegistry:TriggerEvent("BankPanelMixin.PageInfoChanged", 1);
		local numColumns = 14;
		local totalSlots = C_Container.GetContainerNumSlots(self.selectedTabID);
		local isFirstButton = true;
		local currentColumn = 1;
		local slotIDOffset = 0;
		local firstButtonXOffset = 26;
		local ignorePages = true;
		self:GenerateAndPlaceOnePageOfItemButtons(numColumns, totalSlots, self.selectedTabID, isFirstButton, currentColumn, slotIDOffset, firstButtonXOffset, ignorePages);
	end
end
...
function BankPanelMixin:ToggleButtonGlowForItemsOfBankBag(bankBagID, show)
	for itemButton in self:EnumerateValidItems() do
		if(itemButton:GetBankTabID() == bankBagID) then
			itemButton.BagIndicator:SetShown(show);
		end
	end
end
...
function BankPanelMixin:FindItemButtonByBankTabAndContainerSlotID(bankTabID, containerSlotID)
	for itemButton in self:EnumerateValidItems() do
		if itemButton:GetBankTabID() == bankTabID and itemButton:GetContainerSlotID() == containerSlotID then
			return itemButton;
		end
	end
end
...
function BankPanelTabSettingsMenuMixin:UpdateBankTabSettings(itemID)
	-- itemID is optional
	...
	C_Bank.UpdateBankTabSettings(bankType, tabID, tabName, tabIcon, depositFlags, itemID);
end
