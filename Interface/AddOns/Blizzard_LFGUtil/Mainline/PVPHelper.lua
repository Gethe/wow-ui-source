...
function PVPHelperFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "BATTLEFIELDS_SHOW" ) then
		local isArena, battleMasterListID = ...;
		if ( C_AddOns.IsAddOnLoaded("Blizzard_PVPUI") ) then
			...
			PVP_TryQueueForBattlefield(battleMasterListID);
		end
	end
end
...
-------------------------------------------------------------------
-- Try queueing for Battlefield
-------------------------------------------------------------------

function PVP_TryQueueForBattlefield(battleMasterListID)
	-- Only allow the party leader is allowed to queue the group for a battleground
	if ( IsInGroup(LE_PARTY_CATEGORY_HOME) and not UnitIsGroupLeader("player", LE_PARTY_CATEGORY_HOME) ) then
		UIErrorsFrame:AddMessage(ERR_NOT_LEADER, RED_FONT_COLOR:GetRGBA());
		return;
	end
