...
function CompactUnitFrame_UpdateAggroHighlight(frame)
	if ( not frame.optionTable.displayAggroHighlight ) then
		if ( not frame.optionTable.playLoseAggroHighlight and frame.aggroHighlight ) then
...
function DefaultCompactNamePlateFrameSetupInternal(frame, setupOptions, frameOptions)
	...
	if (frameOptions.highlightOverHealthBar) then
		frame.selectionHighlight:SetParent(frame.HealthBarsContainer.healthBar);
	else
		frame.selectionHighlight:SetParent(frame.HealthBarsContainer);
	end
	if (frameOptions.displayAggroHighlight) then
		frame.aggroHighlight:SetParent(frame.HealthBarsContainer);
	end

	if (frameOptions.displayHealPrediction) then
	...
	if (frameOptions.playLoseAggroHighlight) then
...
function DefaultCompactNamePlateFrameAnchorInternal(frame, setupOptions)
	...
	if not customOptions or not customOptions.ignoreNamePoint then
