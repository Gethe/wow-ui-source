...
function PetPaperDollFrame_SetTab(id)
	...
	PanelTemplates_SelectTab(_G["PetPaperDollFrameTab"..id]);
end
...
local function GetTooltipSubStringUpToNewLine(tooltipString, howMuchExtraToInclude)
	local tooltipSubString = tooltipString;
	local newLineIndex = strfind(tooltipString, "|n");
	if newLineIndex and newLineIndex + howMuchExtraToInclude < string.len(tooltipString) then
		return strsub(tooltipString, 1, newLineIndex + howMuchExtraToInclude);
	end

	return tooltipSubString;
end
...
function PetPaperDollFrame_SetStats()
	for i=1, NUM_PET_STATS, 1 do
		...
		-- Second tooltip line
		frame.tooltip2 = _G["DEFAULT_STAT"..i.."_TOOLTIP"] or "";
		if ( i == 1 ) then
			local attackPower = effectiveStat-20;
			frame.tooltip2 = format(frame.tooltip2, attackPower);
		elseif ( i == 2 ) then			
			frame.tooltip2 = format(GetTooltipSubStringUpToNewLine(frame.tooltip2, 1), GetCritChanceFromAgility("pet"));
		elseif ( i == 3 ) then
		...
		elseif ( i == 4 ) then
			if ( UnitHasMana("pet") ) then
				...
				frame.tooltip2 = format(frame.tooltip2, manaGain, max(0, effectiveStat-10), GetSpellCritChanceFromIntellect("pet"));
			else
				frame.tooltip2 = format(GetTooltipSubStringUpToNewLine(frame.tooltip2, 2), GetSpellCritChanceFromIntellect("pet"));
			end
		elseif ( i == 5 ) then
