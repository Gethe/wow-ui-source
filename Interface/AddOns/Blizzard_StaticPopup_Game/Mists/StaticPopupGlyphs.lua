StaticPopupDialogs["CONFIRM_REMOVE_GLYPH"] = {
	...
	OnShow = function(dialog, data)
		...
		if cost == 0 then
			dialog:SetFormattedText(CONFIRM_REMOVE_GLYPH_NO_COST, data.name);
...
StaticPopupDialogs["CONFIRM_GLYPH_PLACEMENT"] = {
	...
	OnShow = function(dialog, data)
		local name, count, _, _, cost = GetGlyphClearInfo();
		if cost == 0 then
			dialog:SetFormattedText(CONFIRM_GLYPH_PLACEMENT_NO_COST, data.name);
		elseif count >= cost then
			dialog:SetFormattedText(CONFIRM_GLYPH_PLACEMENT, data.name, GREEN_FONT_COLOR_CODE, cost, name);
		else
			dialog:SetFormattedText(CONFIRM_GLYPH_PLACEMENT, data.name, RED_FONT_COLOR_CODE, cost, name);
			dialog:GetButton1():Disable();
		end
	end,
	hideOnEscape = 1,
