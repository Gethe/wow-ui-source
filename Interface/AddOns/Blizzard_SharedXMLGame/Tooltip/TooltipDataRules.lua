...
if tbl then
	...
	local function CleanFunction(funcToClean)
		local f = function(...)
			...
			return HandleCleanFunctionCallArgs(tbl.pcallwithenv(funcToClean, tbl, ...));
		end
		setfenv(f, tbl);
		return f;
	end
	...
	Import("string");
	Import("Enum");
	Import("BattlePetToolTip_Show");
	Import("ColorMixin");
	Import("CreateColor");
	Import("GetThreatStatusColor");
