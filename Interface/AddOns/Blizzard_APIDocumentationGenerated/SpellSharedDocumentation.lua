	...
	{
		{
			...
			Fields =
			{
				...
				{ Name = "isActive", Type = "bool", Nilable = false, Documentation = { "False if recharging is not active (ex: at maximum available charges, start time or duration are zero); True otherwise" } },
			},
		},
		{
			...
			Fields =
			{
				...
				{ Name = "isActive", Type = "bool", Nilable = false, Documentation = { "False if cooldown is not active (ex: not enabled, or startTime or duration are 0); True otherwise" } },
				...
				{ Name = "isOnGCD", Type = "bool", Nilable = true, Documentation = { "Whether or not this spell is considered to be on the global cooldown, do not trust this field unless responding to a SPELL_UPDATE_COOLDOWN event" } },
			},
		},
		{
			...
			Fields =
			{
				...
				{ Name = "duration", Type = "number", Nilable = false, Documentation = { "Cooldown duration in seconds if active; 0 if cooldown is inactive" } },
				{ Name = "modRate", Type = "number", Nilable = false, Documentation = { "Rate at which cooldown UI should update" } },
				{ Name = "isActive", Type = "bool", Nilable = false, Documentation = { "False if cooldown is not active (startTime or duration are 0); True otherwise" } },
				{ Name = "shouldReplaceNormalCooldown", Type = "bool", Nilable = false, Documentation = { "True if this cooldown should be displayed in-place of the normal action cooldown (ex: the LoC cooldown expires later than the normal cooldown); False otherwise" } },
			},
		},
		{
