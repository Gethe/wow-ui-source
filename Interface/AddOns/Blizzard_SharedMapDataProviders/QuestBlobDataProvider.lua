...
function QuestBlobPinMixin:TryDrawQuest(questID)
	if questID and questID > 0 then --and (self.dataProvider:IsShowingWorldQuests() or not QuestUtils_IsQuestWorldQuest(questID)) and (C_QuestLog.IsThreatQuest(questID) or not QuestUtils_IsQuestBonusObjective(questID)) then
...
function QuestBlobPinMixin:UpdateTooltip()
	...
	--[[if C_QuestLog.IsThreatQuest(questLogIndex) then
