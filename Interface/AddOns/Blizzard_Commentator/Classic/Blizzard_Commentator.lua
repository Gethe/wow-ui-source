UIModeUtil.RegisterMode("CommentatorSpectating", {
	-- We want commentator to include pvp frames and widgets, which are hidden by hide most normally.
	rolesetBlocklist = UIModeUtil.CreateModifiedBlocklist({ pvp = true, widgets = true }, "chat", "microMenu", "arenaFrames"),
});
...
function CommentatorMixin:SetCommentatorMode(enabled)
	UIModeUtil.SetModeActive("CommentatorSpectating", enabled);
	C_Commentator.SetMouseDisabled(enabled);
end

function CommentatorMixin:ToggleCommentatorMode()
	self:SetCommentatorMode(not UIModeUtil.IsModeActive("CommentatorSpectating"));
end
