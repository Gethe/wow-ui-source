...
local MAX_PET_SLOT_INDEX = Constants.PetConsts.NUM_PET_SLOTS_HUNTER;
...
local function IsExtraPetButtonEnabled()
	return Constants.PetConsts.EXTRA_PET_STABLE_SLOT >= 0;
end

local function IsActivePetSlot(slot)
	if IsExtraPetButtonEnabled() then
		return slot > 0 and slot < EXTRA_PET_STABLE_SLOT_LUA_INDEX;
	end
	return slot > 0 and slot <= Constants.PetConsts.MAX_SUMMONABLE_HUNTER_PETS;
end
...
local function FindFirstUnusedStableSlot()
	...
	local startIndex = STABLED_PETS_FIRST_SLOT_LUA_INDEX;

	if IsExtraPetButtonEnabled() then
		startIndex = startIndex + 1;
	end

	for i = startIndex, MAX_PET_SLOT_INDEX do
		local petInfo = C_StableInfo.GetStablePetInfo(i);
		if not petInfo then
			targetSlot = i;
			break;
	...
	if not targetSlot and IsExtraPetButtonEnabled() then
...
function StableFrameMixin:SetupPetCounter()
	...
	local counterText = STABLE_PET_COUNTER:format(totalNumPets, Constants.PetConsts.NUM_PET_SLOTS_HUNTER);
...
function StableActivePetListMixin:OnLoad()

	if not IsExtraPetButtonEnabled() then
		self.Divider:Hide();
	end
...
function StableActivePetButtonTemplateMixin:IsEnabled()
	if self.BeastMasterSecondaryPetButton and IsExtraPetButtonEnabled() then
		return true;
	end

	return self:GetID() <= Constants.PetConsts.MAX_SUMMONABLE_HUNTER_PETS;
end
...
function StableStabledPetListMixin:OnLoad()
	...
	if Constants.PetConsts.MAX_STABLE_SLOTS <= self.minStabledForFilter then
		self.FilterBar:Hide();
	end
...
function StablePetSpecializationMixin:OnLoad()
	...
	if #C_StableInfo.GetAvailablePetSpecInfos() <= 1 then
		self:Hide();
		return;
	end
