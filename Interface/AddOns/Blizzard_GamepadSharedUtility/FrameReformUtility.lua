...
--[[
	Adds a highlight texture to a square button

	itemFrame: frame to button highlight on
]]
function GamepadMode.AddHighlightTexture(itemFrame)
	assert(type(itemFrame) == "table", "Param itemFrame is invalid type");

	itemFrame.GamepadHighlightTexture = itemFrame:CreateTexture();
	itemFrame.GamepadHighlightTexture:SetTexture("Interface\\Buttons\\UI-Button-Outline");
	itemFrame.GamepadHighlightTexture:SetPoint("CENTER", itemFrame, "CENTER");

	if (itemFrame.SetHighlightTexture) then
		itemFrame:SetHighlightTexture(itemFrame.GamepadHighlightTexture);
	end
end

--[[
	Adds a highlight texture to a frame, given the highlight texture file

	itemFrame: frame to button highlight on
	textureFile: file path to the highlight texture to use (can be nil if atlasName is used)
	atlasName: atlas index for texture file, can be left nil if textureFile isn't
]]
function GamepadMode.AddCustomHighlightTexture(itemFrame, textureFile, atlasName)
	assert(type(itemFrame) == "table", "Param itemFrame is invalid type");
	assert(type(textureFile) == "string" or type(atlasName) == "string", "Param textureFile or atlasName must be a string");

	itemFrame.GamepadHighlightTexture = itemFrame:CreateTexture();
	if (textureFile) then
		itemFrame.GamepadHighlightTexture:SetTexture(textureFile);
	else
		itemFrame.GamepadHighlightTexture:SetAtlas(atlasName);
	end
	itemFrame.GamepadHighlightTexture:SetAllPoints();
	itemFrame.GamepadHighlightTexture:SetDrawLayer("HIGHLIGHT");
...
--[[
	Creates an icon button with text, updated and better than CreateDownClickButtonWithText

	name: Name for the button
	onClickFunction: Function to run on left mouse click (optional)
...
function GamepadMode.CreateButtonWithTextFromTemplate(name, parent, onClickFunction, text, iconNames)
	...
	assert(not onClickFunction or type(onClickFunction) == "function", "Param onClickFunction is wrong type");

	if (not onClickFunction) then
		onClickFunction = function() end
	end
	local button = CreateFrame("BUTTON", name, parent, "GamepadButtonIconWithTextTemplate");
	...
	button:HookScript("OnClick", onClickFunction);
