...
function CapProgressBar_Update(capBar, cap1Quantity, cap1Limit, cap2Quantity, cap2Limit, totalQuantity, totalLimit, hasNoSharedStats)
	...
	if ( cap2Quantity and cap2Limit ) then
		cap1Width = min(cap1Limit - cap1Quantity, cap2Limit - cap2Quantity) * sizePerPoint;
...
local function GetCurrencyHyperlink(linkID)
	if C_CurrencyInfo and C_CurrencyInfo.GetCurrencyLink then
		return C_CurrencyInfo.GetCurrencyLink(linkID);
	end

	return GetCurrencyLink(linkID);
end
...
function InlineHyperlinkFrame_OnClick(self, link, text, button)
	if ( self.hasIconHyperlinks ) then
		...
		if ( linkType == "currency" ) then
			fixedLink = GetCurrencyHyperlink(linkID);
		end
...
function CurrencyTemplateMixin:OnEnter()
	if self.tooltipAnchor and self.currencyID and self.Text and GameTooltip.SetCurrencyByID then
