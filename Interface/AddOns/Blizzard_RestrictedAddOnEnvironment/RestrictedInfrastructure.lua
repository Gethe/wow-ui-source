...
function GetFrameHandleFrame(handle, protected, onlyProtected)
	...
	if ((surrogate == nil) and (not protected or (not (onlyProtected or InCombatLockdown())))) then
		surrogate = LOCAL_FrameHandle_Other_Frames[handle];
...
function InitFrameHandleNamespace(namespace)
	if (not handleNamespaceInitialized) then
		...
		for k, v in pairs(namespace) do
			if (type(k) == "string" and type(v) == "function" and issecure()) then
				HANDLE[k] = v;
			end
		end
	end
...
getmetatable(LOCAL_Readonly_Restricted_Tables).__index = RestrictedTable_Readonly_index;
...
local function RestrictedTable_insert(T, ...)
	...
	if (PT) then
		...
		local pos, val = ...;
		pos = tonumber(pos);
		if issecretvalue(pos) then
			error("Attempted to use a secret value as a table index in an insert operation");
			return;
		end
		t_insert(PT, pos, nil);
...
local function RestrictedTable_remove(T, pos)
	...
	if (PT) then
		...
		if issecretvalue(pos) then
			error("Attempted to use a secret value as a table index in a remove operation");
			return;
		end
