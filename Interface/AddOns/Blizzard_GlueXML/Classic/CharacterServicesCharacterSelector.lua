...
function CharacterServicesCharacterSelectorMixin:UpdateDisplay(block)
	...
	self.GlowBox:SetHeight(58 * numDisplayedCharacters);
	self.GlowBox:SetPoint("TOP", CharacterSelectCharacterFrame, -11, -64);
	self.GlowBox:SetWidth(238);
	...
	CharacterSelect_HidePaidServiceButtons();
...
function CharacterServicesCharacterSelectorMixin:Refresh()
	CharacterSelect_ForEachVisibleCharacterButton(function(button)
		self:UpdateSingleCharacter(button);
	end);
end
...
function CharacterServicesCharacterSelectorMixin:ResetState(selectedButtonIndex)
	...
	local button = CharacterSelect_FindCharacterButtonByPredicate(function(frame, elementData)
