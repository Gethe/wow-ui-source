...
function AddOrDeleteHairColorForWorgens(raceID)
	--Worgens don't have a hair color, so they need special functionality
	local hairColorOption = _G["CharacterCustomizationButtonFrame"..HAIR_COLOR_OPTION_INDEX];
	local nextOption = _G["CharacterCustomizationButtonFrame"..HAIR_COLOR_OPTION_INDEX + 1];
	local prevOption = _G["CharacterCustomizationButtonFrame"..HAIR_COLOR_OPTION_INDEX - 1];
	if (raceID == WORGEN_RACE_ID and not C_CharacterCreation.IsViewingAlteredForm()) then
    
		hairColorOption:Hide();
		if(HAIR_COLOR_OPTION_INDEX < NUM_CHAR_CUSTOMIZATIONS) then
			if(HAIR_COLOR_OPTION_INDEX == 1) then
				nextOption:SetPoint("TOP", _G["CharacterCreateGenderButtonMale"], "BOTTOM", 15, -10);
			else
				nextOption:SetPoint("TOP", prevOption, "BOTTOM");
			end
		end	
	else
		hairColorOption:Show();
		if(HAIR_COLOR_OPTION_INDEX < NUM_CHAR_CUSTOMIZATIONS) then
			nextOption:SetPoint("TOP", hairColorOption, "BOTTOM");
		end
	end
end
...
function SetCharacterRace(id)
	...
	AddOrDeleteHairColorForWorgens(id);
...
function SetViewingAlteredForm()
	local isViewingAlteredFormInitially = C_CharacterCreation.IsViewingAlteredForm();
	C_CharacterCreation.SetViewingAlteredForm(not isViewingAlteredFormInitially);
	CharacterCreate_UpdateCustomizationOptions();
	CharacterCreateAlternateFormTop:SetChecked(isViewingAlteredFormInitially);
	CharacterCreateAlternateFormBottom:SetChecked(not isViewingAlteredFormInitially);
	AddOrDeleteHairColorForWorgens(C_CharacterCreation.GetSelectedRace());
end
