...
function PVPTeam_Update()
	...
	for index, value in pairs(ARENA_TEAMS) do
		if ( value.index ) then
			...
			-- Pull Values
			teamName, teamSize, teamRating, teamPlayed, teamWins,  playerPlayed, teamRank, background.r, background.g, background.b, emblem, emblemColor.r, emblemColor.g, emblemColor.b, border, borderColor.r, borderColor.g, borderColor.b = GetArenaTeam(value.index);
	...
	for index, value in pairs(ARENA_TEAMS) do
		if ( not value.index ) then
			...
			getglobal(buttonName.."TeamType"):SetText(format(PVP_TEAMSIZE, value.size, value.size));
...
function PVPTeamDetails_OnShow()
	CloseWindows(nil, CharacterFrame);
	PlaySound("igSpellBookOpen");
end

function PVPTeamDetails_Update(id)
	local numMembers = GetNumArenaTeamMembers(id, 1);
	local name, rank, level, class, online, played, win, loss, seasonPlayed, seasonWin, seasonLoss;
	local teamName, teamSize, teamRating, teamPlayed, teamWins, teamRank  = GetArenaTeam(id);
	local button;
	local teamIndex;
	...
	PVPTeamDetailsSize:SetText(format(PVP_TEAMSIZE, teamSize, teamSize));
	PVPTeamDetailsStatsType:SetText("THIS WEEK");
	...
	for i=1, MAX_ARENA_TEAM_MEMBERS, 1 do
		...
		if ( i > numMembers ) then
			...
			button.teamIndex = i;
			-- Get Data
			name, rank, level, class, online, played, win, seasonPlayed, seasonWin = GetArenaTeamRosterInfo(id, i);
			loss = played - win;
			seasonLoss = seasonPlayed - seasonWin;
			button.tooltip = LEVEL.." "..level.." "..class;

			-- Populate Data into the display, season or this week
			if ( PVPTeamDetails.statsType ) then
				getglobal("PVPTeamDetailsButton"..i.."Name"):SetText(name);
				getglobal("PVPTeamDetailsButton"..i.."Class"):SetText(class);
				getglobal("PVPTeamDetailsButton"..i.."Played"):SetText(seasonPlayed);
				getglobal("PVPTeamDetailsButton"..i.."Win"):SetText(seasonWin);
				getglobal("PVPTeamDetailsButton"..i.."Loss"):SetText(seasonLoss);
				PVPTeamDetailsFrameColumnHeader3.sortType = "seasonplayed";
				PVPTeamDetailsFrameColumnHeader4.sortType = "seasonwon";
				PVPTeamDetailsStatsType:SetText(ARENA_THIS_SEASON);
				PVPTeamDetailsToggleButton:SetText(ARENA_THIS_WEEK_TOGGLE);
			else
				getglobal("PVPTeamDetailsButton"..i.."Name"):SetText(name);
				getglobal("PVPTeamDetailsButton"..i.."Class"):SetText(class);
				getglobal("PVPTeamDetailsButton"..i.."Played"):SetText(played);
				getglobal("PVPTeamDetailsButton"..i.."Win"):SetText(win);
				getglobal("PVPTeamDetailsButton"..i.."Loss"):SetText(loss);
				PVPTeamDetailsFrameColumnHeader3.sortType = "played";
				PVPTeamDetailsFrameColumnHeader4.sortType = "won";
				PVPTeamDetailsStatsType:SetText(ARENA_THIS_WEEK);
			...
			if ( online ) then
				if ( rank > 0 ) then
					getglobal("PVPTeamDetailsButton"..i.."Name"):SetTextColor(1.0, 1.0, 1.0);
				else
					getglobal("PVPTeamDetailsButton"..i.."Name"):SetTextColor(1.0, 0.82, 0.0);
				end
				getglobal("PVPTeamDetailsButton"..i.."Class"):SetTextColor(1.0, 1.0, 1.0);
				getglobal("PVPTeamDetailsButton"..i.."Played"):SetTextColor(1.0, 1.0, 1.0);
				getglobal("PVPTeamDetailsButton"..i.."Win"):SetTextColor(1.0, 1.0, 1.0);
				getglobal("PVPTeamDetailsButton"..i.."Loss"):SetTextColor(1.0, 1.0, 1.0);
			else
				getglobal("PVPTeamDetailsButton"..i.."Name"):SetTextColor(0.5, 0.5, 0.5);
				getglobal("PVPTeamDetailsButton"..i.."Class"):SetTextColor(0.5, 0.5, 0.5);
				getglobal("PVPTeamDetailsButton"..i.."Played"):SetTextColor(0.5, 0.5, 0.5);
				getglobal("PVPTeamDetailsButton"..i.."Win"):SetTextColor(0.5, 0.5, 0.5);
				getglobal("PVPTeamDetailsButton"..i.."Loss"):SetTextColor(0.5, 0.5, 0.5);
			end
...
function PVPTeamDetailsToggleButton_OnClick()
	if ( PVPTeamDetails.statsType ) then
		PVPTeamDetails.statsType = nil;
	else
		PVPTeamDetails.statsType = 1;		
	end
	PVPTeamDetails_Update(PVPTeamDetails.team);
end
...
function PVPHonor_Update()
	...
	PVPHonorTodayHonor:SetWidth(PVPHonorTodayHonor:GetWidth());
