...
function LFMFrame_OnEvent(event)
	...
	elseif ( event == "PARTY_LEADER_CHANGED" ) then
		if ( IsPartyLeader() and ((GetNumPartyMembers() > 0) or (GetNumRaidMembers() > 0)) ) then
			LFGFrame.loaded = nil;
			LFMFrame_Update();
			SendLFGQuery();
		end
...
function LFMFrame_Update()
	...
	if ( lfmStatus and ((GetNumPartyMembers() == 0) or IsPartyLeader())) then
	...
	elseif ( (GetNumPartyMembers() == 0) or not LFGFrame.loaded) then
...
function LFMFrame_OnUpdate(elapsed)
	...
	if ( LFMFrameSearchButton.refreshTimer >= LFM_REFRESH_UPDATE_THROTTLE ) then
		...
		local _, _, _, _, _, _, _, _, _, queued, lfgStatus, lfmStatus = GetLookingForGroup();
		if ( (queued and lfgStatus) or PartyIsFull() or (((GetNumPartyMembers() > 0) and not IsPartyLeader()) or ((GetNumRaidMembers() > 0) and not IsRaidLeader())) ) then
