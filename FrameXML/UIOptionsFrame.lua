...
-- Display
UIOptionsFrameCheckButtons["XP_BAR_TEXT"] =					{ index = 2,  cvar = "xpBarText" };
UIOptionsFrameCheckButtons["STATUS_TEXT_PLAYER"] =			{ index = 81,  cvar = "playerStatusText" };
UIOptionsFrameCheckButtons["STATUS_TEXT_PET"] =				{ index = 82,  cvar = "petStatusText" };
UIOptionsFrameCheckButtons["STATUS_TEXT_PARTY"] =			{ index = 83,  cvar = "partyStatusText" };
UIOptionsFrameCheckButtons["STATUS_TEXT_TARGET"] =			{ index = 84,  cvar = "targetStatusText" };
UIOptionsFrameCheckButtons["STATUS_TEXT_PERCENT"] =			{ index = 85,  cvar = "statusTextPercentage" };
...
UIOptionsFrameCheckButtons["SHOW_FULLSCREEN_STATUS_TEXT"] =	{ index = 80, uvar = "SHOW_FULLSCREEN_STATUS", default = "1"};
...
function UIOptionsFrame_Init()
	...
	SHOW_PARTY_TEXT = "0";
	RegisterForSave("SHOW_PARTY_TEXT");
	-- Combat text uvars
	SHOW_COMBAT_TEXT = "0";
	...
	BasicOptionsStatusText:SetPoint("TOPRIGHT", BasicOptionsGeneral, "BOTTOMRIGHT", 0, -20);
	BasicOptionsDisplay:SetPoint("TOPLEFT", BasicOptionsGeneral, "BOTTOMLEFT", 0, -20);
	BasicOptionsDisplay:SetPoint("TOPRIGHT", BasicOptionsStatusText, "TOPLEFT", -8, 0);
...
function UIOptionsFrame_Load()
	...
	UIDropDownMenu_SetSelectedValue(UIOptionsFrameAutoLootKeyDropDown, GetModifiedClick("AUTOLOOTTOGGLE"));
	UIDropDownMenu_EnableDropDown(UIOptionsFrameAutoLootKeyDropDown);
	UIDropDownMenu_SetText(getglobal(GetModifiedClick("AUTOLOOTTOGGLE").."_KEY"), UIOptionsFrameAutoLootKeyDropDown);
...
function UIOptionsFrame_Save()
	...
	for index, value in pairs(UIOptionsFrameCheckButtons) do
		...
		elseif ( value.uvar == "SHOW_BUFF_DURATIONS" ) then
			setglobal(value.uvar, value.value);
			BuffButtons_UpdatePositions();
		elseif ( value.uvar == "SHOW_PARTY_TEXT" ) then
			setglobal(value.uvar, value.value);
			PartyMemberFrame_UpdateStatusBarText();
		elseif ( value.uvar ) then
			setglobal(value.uvar, value.value);
	...
	-- Save auto loot key
	local binding = UIDropDownMenu_GetSelectedValue(UIOptionsFrameAutoLootKeyDropDown);
	if ( binding ~= GetModifiedClick("AUTOLOOTTOGGLE") ) then
		SetModifiedClick("AUTOLOOTTOGGLE", binding);
		SaveBindings(GetCurrentBindingSet());
	end
	-- For Locale
	if ( UIOptionsFrameLocaleDropDown_CountExistingLocales(GetExistingLocales()) > 1 and
		(GetCVar("locale") ~= UIDropDownMenu_GetSelectedValue(UIOptionsFrameLocaleDropDown)) ) then
...
function UIOptionsFrameLocaleDropDown_OnLoad()
	...
	if ( UIOptionsFrameLocaleDropDown_CountExistingLocales(GetExistingLocales()) <= 1 ) then
...
function UIOptionsFrameLocaleDropDown_CountExistingLocales(...)
	local count = 0;
	for i = 1, select("#", ...) do
		if (select(i, ...)) then
			count = count + 1;
...
function UIOptionsFrameLocaleDropDown_Initialize()
	...
	UIOptionsFrameLocaleDropDown_InitializeHelper(info, selectedValue, GetExistingLocales());
end

function UIOptionsFrameLocaleDropDown_InitializeHelper(createInfo, selectedValue, ...)
	for i = 1, select("#", ...) do
		local value = select(i, ...);
		if (value) then
			createInfo.text = getglobal(strupper(value));
			createInfo.func = UIOptionsFrameLocaleDropDown_OnClick;
...
function UIOptionsFrame_SetDefaults()
	...
	UIOptionsFrameAutoLootKeyDropDown.tooltip = getglobal("OPTION_TOOLTIP_AUTO_LOOT_SHIFT_KEY");
